...
if main_options == 'About':
    ...
    st.write('''
                      Linkedin:- https://www.linkedin.com/in/yamini-peddireddi123/

                      ''')
elif main_options == 'Detection space':

    option = st.radio('Which type of detection would you like to make?', ('an Image', 'a Video', 'OpenCV Live', 'a Live Video detection'))
    ...
    if option == 'an Image':
        ...
        st.write('For real time detection in streamlit please use a Live Video detection option for finding out the emotion of a person in live')
        st.write('Thank you')
        ...
        def face_detect():
            ...
            ctx = webrtc_streamer(key="example", mode=WebRtcMode.SENDRECV, video_transformer_factory=VideoTransformer, async_transform=True)
        ...
        st.write('This is running using newly introduced webrtc tool which can access the camera whereas opencv cannot function properly in streamlit')
        st.write('This new tool takes some time for detecting your live video,please wait for few minutes(2-3 mins max)')
